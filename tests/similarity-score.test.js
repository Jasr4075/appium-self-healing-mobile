...
test('similarityScore: calculate gera text-similar para texto parecido', () => {
  const result = similarityScore.calculate('android=new UiSelector().text("Entrar no sistema")', {
    text: 'Entrar no app',
    index: 0,
  });
...
test('similarityScore: calculate usa id-partial para IDs com parte comum', () => {
  const result = similarityScore.calculate('android=new UiSelector().resourceId("btn_ok")', {
    id: 'com.app:id/btn_ok',
    index: 0,
  });
